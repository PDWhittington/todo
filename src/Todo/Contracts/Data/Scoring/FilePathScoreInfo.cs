...
public record FilePathScoreInfo : ScoreInfo
{
    ...
    public override int Total() 
        => _scores.Sum(x => x.Value);
    
    public override int GetScore(ScoreCategory category) 
        => _scores[category];
    
    public override bool TryGetScore(ScoreCategory category, out int score) 
        => _scores.TryGetValue(category, out score);

    private FilePathScoreInfo(DayListFilePathInfo filePathInfo, IEnumerable<KeyValuePair<ScoreCategory, int>> scores)
    {
    ...
    public static FilePathScoreInfo Of(DayListFilePathInfo filePathInfo, 
